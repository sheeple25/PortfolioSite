...
export type WritingFrontmatter = {
  ...
  coverAlt?: string;
  /**
   * A cover composed for a small, roughly square card, where `cover` is
   * composed for a full-bleed banner. Optional, and only worth setting when the
   * two genuinely disagree.
   *
   * Vashi is the case that forced it: its cover is a flat plate with the mark
   * deliberately high in the frame, because the case study's banner crops it
   * with `object-position: 50% 35%` and a centred mark sat under the title
   * plate. The work graph's card crops nothing vertically, so the same asset
   * puts the mark at 29% of the card's height. Moving it would fix one and
   * break the other; a second asset fixes both. Generated by
