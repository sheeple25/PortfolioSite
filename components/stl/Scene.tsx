 ...
 * `StlViewer.tsx`, so that file can `next/dynamic` it with `ssr: false` —
 * three.js touches WebGL/canvas APIs that don't exist during server rendering,
 * and @react-three/fiber's Canvas assumes a browser environment on mount.
 *
 * Import `StlViewer`, not this. Mounting a bare `Canvas` from a server-rendered
 * tree is the exact failure the split exists to prevent.
 */
...
export default function StlScene({ modelUrl }: { modelUrl: string }) {
